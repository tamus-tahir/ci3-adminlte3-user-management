...
{
    function cekDetailAkses()
    {
        $where = ['url' => $this->uri->segment(1)];
        $page = $this->Default_m->getWhere('tabel_navigasi', $where)->row();

        $where = ['id_navigasi' => $page->id_navigasi, 'id_profil' => $this->session->userdata('id_profil')];
        return $this->Default_m->getWhere('tabel_akses', $where)->row();
    }
    ...
    function getSearch($table, $field, $like, $order)
    {
    ...
    function getWhereOrderTwoTableAsc($table1, $table2, $on, $where, $order)
    {
        $this->db->join($table2, $on);
